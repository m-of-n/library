...
`schema/record.schema.yaml` is the schema. `docs/construction.md`
is the method. This skill is the judgment around them.

1. **Grep `records/` first.** Never create a second record for the same
...
3. `bin/ingest <type> <source> --topic <id> --bears-on <ids>`. Never
